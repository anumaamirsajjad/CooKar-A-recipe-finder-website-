...
def add_recipe():
    ...
    for idx, ing in enumerate(data.get("ingredients", [])):
        ...
        existing_ing = ingredients_collection.find_one({"name": ing["name"]})
        if existing_ing:
            ingredient_ids.append(existing_ing["_id"])
        else:
            ...
            ingredient_ids.append(res.inserted_id)
    ...
    for name in cuisine_names:
        existing = cuisine_collection.find_one({"name": name})
        if existing:
            cuisine_ids.append(existing["_id"])
        else:
            res = cuisine_collection.insert_one({"name": name})
            cuisine_ids.append(res.inserted_id)
    ...
    try:
        recipe_doc = {
            "title": data.get("title"),
            "ingredients": ingredient_ids,              # store as ObjectIds
            "instructions": data.get("instructions"),
            "servings": int(data.get("servingSize")),   # store as integer
            "image": data.get("image"),
        ...
        # Fetch and enrich before returning
        created = recipe_collection.find_one({"_id": result.inserted_id})
        created = enrich_recipe(created, db)
        return Response(dumps(created), mimetype="application/json"), 201
